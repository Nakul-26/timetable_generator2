...
function generate({
  ...
  function schedule_slot(day, hour) {
    ...
    // --- Progress Reporting ---
    if (progressCallback) {
      // A simple progress estimate based on slots processed
...
function countTotalRequiredHoursForClass(classId) {
  ...
  if (!ok) {
    ...
    return {
      ...
      subject_hours_assigned_per_class,
      allocations_report, // Include in return
    };
  }
