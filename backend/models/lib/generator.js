...
function generate({
  ...
  function can_place_combined(combo, subj, group, day, hour, blockSize) {
    ...
    const facultyDayHours = faculty_timetables[combo.faculty_id][day].filter((x) => x !== EMPTY_SLOT && x !== BREAK_SLOT).length;
  ...
  function is_placement_valid(classId, day, hour, combo_id, blockSize = 1) {
    ...
    if (!check_continuous_hours(combo.faculty_id, day, hour, blockSize)) {
      if (DEBUG) console.log(`[REJECT] Too many continuous hours: Faculty ${combo.faculty_id} D${day+1} H${hour+1}`);
      return false;
    }
    ...
    for (const { combo, subj } of candidates) {
      ...
      if (isCombined) {
        ...
        if (is_placement_valid(classId, day, hour, combo_id, blockSize)) {
          if (blockSize > 1 && !can_place_lab_block(classId, combo.faculty_id, day, hour, blockSize)) continue;
          for (let h = hour; h < hour + blockSize; h++) {
