...
console.log('Generator module loaded and debug is', DEBUG ? 'ON' : 'OFF');
...
function generate({
  ...
  const slotAlternatives = new Map();
  ...
  function undo_slot(classId, day, hour) {
    const combo_id = class_timetables[classId][day][hour];
    if (combo_id === EMPTY_SLOT || combo_id === "BREAK") return;

    const combo = comboByMongoId.get(combo_id);
    const subj = subjectById.get(combo.subject_id);
    const blockSize = subj.type === 'lab' ? 2 : 1;
    
    // Find the start of the block to correctly undo multi-hour slots
    let startHour = hour;
    while(startHour > 0 && class_timetables[classId][day][startHour - 1] === combo_id) {
      startHour--;
    }
  ...
  function try_local_repair(day, hour) {
    if (day === 0 && hour === 0) return false;

    const prevHour = hour > 0 ? hour - 1 : HOURS_PER_DAY - 1;
    const prevDay = hour > 0 ? day : day - 1;

    if (prevDay < 0) return false;

    for (const classId of classIds) {
        const key = `${classId}|${prevDay}|${prevHour}`;
        const alternatives = slotAlternatives.get(key);
        if (!alternatives || alternatives.length <= 1) continue;

        const currentComboId = class_timetables[classId][prevDay][prevHour];
        if (currentComboId === EMPTY_SLOT || currentComboId === "BREAK") continue;

        const currentSubj = subjectById.get(comboByMongoId.get(currentComboId)?.subject_id);
        const currentBlockSize = currentSubj?.type === 'lab' ? 2 : 1;

        for (const altComboId of alternatives) {
            if (altComboId === currentComboId) continue;
            
            const altCombo = comboByMongoId.get(altComboId);
            const altSubj = subjectById.get(altCombo?.subject_id);
            const altBlockSize = altSubj?.type === 'lab' ? 2 : 1;

            undo_slot(classId, prevDay, prevHour);

            if (is_placement_valid(classId, prevDay, prevHour, altComboId, altBlockSize)) {
                apply_slot(classId, prevDay, prevHour, altComboId, altBlockSize);

                if (schedule_slot(day, hour)) return true;
            }
            
            // Backtrack the repair attempt
            undo_slot(classId, prevDay, prevHour); 
            apply_slot(classId, prevDay, prevHour, currentComboId, currentBlockSize);
        }
    }
    return false;
  }
  ...
  function teacher_feasible(faculty_ids, day) {
    for (const fid of faculty_ids) {
      ...
      if (assigned > 0) {
        if (assigned > 4) {
            // console.log(`❌ Reject reason: Teacher ${fid} already has more than 4 hours on day ${day}`);
            return false;
        }
        // if (assigned + remaining < 4) {
        //     // console.log(`❌ Reject reason: Teacher ${fid} cannot reach 4 hours on day ${day}`);
        //     return false;
        // }
      }
    }
  ...
  function is_placement_valid(classId, day, hour, combo_id, blockSize = 1) {
    ...
    const subj = subjectById.get(combo.subject_id);
    console.log(`[CHECK] Class=${classById.get(classId)?.name}, Day=${day}, Hour=${hour}, Subject=${subj.name}, Faculty=${combo.faculty_ids.join(",")}`);

    if (BREAK_HOURS.includes(hour)) {
      console.log("❌ Reject reason: Break slot");
      return false;
    }
    for (let h = hour; h < hour + blockSize; h++) {
      const fixed = getFixedSlot(classId, day, h);
      if (fixed && fixed.comboId !== combo._id) {
        console.log("❌ Reject reason: Fixed slot mismatch");
        return false;
      }
    }
    ...
    if (assigned >= required) {
        console.log("❌ Reject reason: Subject hours already met");
        return false;
    }
    if (class_timetables[classId][day][hour] !== EMPTY_SLOT) {
      console.log("❌ Reject reason: Class slot busy");
      return false;
    }
    // CRITICAL FIX: Check if any faculty in the combo is already busy at this slot
    for (const facultyId of combo.faculty_ids) {
      if (faculty_timetables[facultyId][day][hour] !== EMPTY_SLOT) {
        console.log(`❌ Reject reason: Faculty ${facultyId} already busy`);
        return false;
      }
    }
    for (const facultyId of combo.faculty_ids) {
        ...
        if (facultyDayHours + blockSize > 4) {
            console.log(`❌ Reject reason: Faculty ${facultyId} would exceed max daily hours`);
            return false;
        }
    }
    if (!check_continuous_hours(combo.faculty_ids, day, hour, blockSize)) {
      console.log(`❌ Reject reason: Exceeds max continuous hours for a faculty`);
      return false;
    }
    if (!subject_feasible(classId, subj._id)) {
        console.log(`❌ Reject reason: Subject ${subj.name} not feasible for class ${classById.get(classId)?.name}`);
        return false;
    }
    return true;
  }
  ...
  function schedule_slot(day, hour, repair_depth = 0) {
    CALLS++;
    if (CALLS % 1000 === 0) {
      console.log("CALLS =", CALLS, "at", day, hour);
    }
    if (repair_depth > MAX_REPAIR_DEPTH) return false;
    if (day === MAX_DAYS - 1 && hour === HOURS_PER_DAY - 1) {
      console.log("⚠️ FINAL SLOT STATE");
      console.log("Remaining hours:", total_remaining_required_hours());
    }
    console.log(`\n➡️ schedule_slot(Day=${day}, Hour=${hour}, RepairDepth=${repair_depth})`);
    ...
    if (hour >= HOURS_PER_DAY) return schedule_slot(day + 1, 0, repair_depth);
    if (BREAK_HOURS.includes(hour)) return schedule_slot(day, hour + 1, repair_depth);
    if (progressCallback) {
      ...
      const partial_allocations_report = {};
      for (const cls of classes) {
        if (cls.total_class_hours > 0) {
          const report = get_class_allocation_report_data(cls._id);
          if (report) partial_allocations_report[cls._id] = report;
        }
      }
      progressCallback({
        progress: Math.min(100, Math.round(progress)),
        partialData: { class_timetables, faculty_timetables, subject_hours_assigned_per_class, combos, allocations_report: partial_allocations_report },
      });
    }
    if (!optimistic_feasible(day, hour)) return false;
    const classOrder = get_class_order_for_slot(day, hour);
    console.log(
      `Trying slot Day=${day} Hour=${hour} for classes:`,
      classOrder.map(cid => classById.get(cid)?.name)
    );
    return try_class_in_slot(day, hour, classOrder, 0, repair_depth);
  }
  ...
  function destroy_class_days(classId, days) {
    const removed = [];
    ...
    for (const d of days) {
      ...
      for (let h = 0; h < HOURS_PER_DAY; h++) {
        ...
        const combo_id = class_timetables[classId][d][h];
        if (combo_id !== EMPTY_SLOT && combo_id !== "BREAK") {
          
          if (getFixedSlot(classId, d, h)) continue;
          ...
          let startHour = h;
          while (startHour > 0 && class_timetables[classId][d][startHour - 1] === combo_id) {
            startHour--;
          }
  
          removed.push({ classId, day: d, hour: startHour, combo_id, blockSize });
  
          for (let i = 0; i < blockSize; i++) {
            const currentHour = startHour + i;
            if (currentHour < HOURS_PER_DAY) {
              undo_slot(classId, d, currentHour);
              processedSlots.add(`${d}-${currentHour}`);
            }
          }
        }
      }
  ...
  function restore_removed(removed) {
    for (const r of removed) {
      apply_slot(r.classId, r.day, r.hour, r.combo_id, r.blockSize);
    }
  }

  function try_large_repair(currentDay, currentHour, stuckClassId) {
    const days_to_destroy = [];
    if (currentDay > 0) days_to_destroy.push(currentDay - 1);
    if (currentDay > 1) days_to_destroy.push(currentDay - 2);
  
    if (days_to_destroy.length > 0) {
      console.log(`--- Attempting Large Repair for class ${stuckClassId} on days ${days_to_destroy.join(',')} ---`);
  
      const removed = destroy_class_days(stuckClassId, days_to_destroy);
      if (removed.length === 0) return false; 
  
      const repair_start_day = Math.min(...days_to_destroy);
  
      if (schedule_slot(repair_start_day, 0)) {
        console.log(`--- Large Repair SUCCESSFUL ---`);
        return true;
      } else {
        console.log(`--- Large Repair FAILED, rolling back ---`);
  ...
  function try_class_in_slot(day, hour, classOrder, classIndex) {
    ...
    candidates.sort((a, b) => {
      const aIsElective = a.subj.type === "elective";
      const bIsElective = b.subj.type === "elective";
      if (aIsElective !== bIsElective) return bIsElective - aIsElective;

      const aIsCombined = a.combo.class_ids.length > 1;
      const bIsCombined = b.combo.class_ids.length > 1;
      ...
      return remA - remB; // smallest remaining FIRST
    });
    
    const key = `${classId}|${day}|${hour}`;
    slotAlternatives.set(key, candidates.map(c => String(c.combo._id)));
    ...
    for (const { combo, subj } of candidates) {
      ...
      if (isCombined) {
        ...
        if (can_place_combined(combo, subj, comboDef.class_ids, day, hour, blockSize)) {
          // Apply slot for all combined classes
          for (const cId of comboDef.class_ids) {
            apply_slot(cId, day, hour, combo_id, blockSize);
          }
          
          if (try_class_in_slot(day, hour, classOrder, classIndex + 1)) return true;

          // Undo slot for all combined classes
          for (const cId of comboDef.class_ids) {
            undo_slot(cId, day, hour);
          }
        }
      } else {
        if (is_placement_valid(classId, day, hour, combo_id, blockSize)) {
          ...
          apply_slot(classId, day, hour, combo_id, blockSize);
          
          if (try_class_in_slot(day, hour, classOrder, classIndex + 1)) return true;
          
          undo_slot(classId, day, hour);
        }
      }
    }
    console.log(`❌ No valid combo for class ${cls.name} at Day=${day}, Hour=${hour}`);
    if (try_local_repair(day, hour)) return true;

    if (total_remaining_required_hours() <= 3) {
      if(try_large_repair(day, hour, classId)) return true;
    }
