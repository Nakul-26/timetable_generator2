...
function subject_feasible(classId, subjId) {
  ...
  function can_place_combined(combo, subj, group, day, hour, blockSize) {
    ...
    if (!subject_feasible(group[0], subj._id)) return false;
  ...
  // Find elective group for a class + subject
  function findElectiveGroup(classId, subjectId) {
    const cls = classById.get(classId);
    if (!cls || !Array.isArray(cls.elective_groups)) return null;
    return cls.elective_groups.find(g => g.subjects.includes(subjectId)) || null;
  }
  ...
  function generate_schedule_for_slot(day, hour, classIndex) {
    ...
    for (const cand of candidates) {
      ...
      if (subj.type === "lab") {
        ...
        // --- PARALLEL ELECTIVE LOGIC ---
        const electiveGroup = findElectiveGroup(classId, subj._id);
        if (electiveGroup) {
          const g = electiveGroup;

          // Ensure all electives in group remain
          const allCandidates = [];
          for (const sid of g.subjects) {
            const required = get_required_hours_for_class_subject(classId, sid);
            const assigned = subject_hours_assigned_per_class[classId][sid] || 0;
            if (assigned >= required) {
              allCandidates.length = 0;
              break;
            }
            const comboIdForSid = resolveComboForSubjectAndClass({
              classId,
              subjectId: sid,
            });
            if (!comboIdForSid) {
            ...
            const cb2 = comboByMongoId.get(comboIdForSid);
            if (!cb2) {
              allCandidates.length = 0;
              break;
            }
            allCandidates.push({
              combo_id: comboIdForSid,
              combo: cb2,
              subj: subjectById.get(sid),
            });
          }

          if (allCandidates.length === g.subjects.length) {
            let valid = true;
            const placedData = [];

            for (const cnd2 of allCandidates) {
              const c2 = cnd2.combo;
              if (!is_placement_valid(classId, day, hour, c2._id, 1)) {
                valid = false;
                break;
              }
              if (faculty_timetables[c2.faculty_id][day][hour] !== EMPTY_SLOT) {
                valid = false;
                break;
              }
              const req2 = get_required_hours_for_class_subject(classId, cnd2.subj._id);
              const asg2 = subject_hours_assigned_per_class[classId][cnd2.subj._id] || 0;
              if (asg2 >= req2) {
                valid = false;
                break;
              }
              placedData.push(cnd2);
            }

            if (valid) {
              for (const cnd2 of allCandidates) {
                if (!teacher_feasible(cnd2.combo.faculty_id, day)) {
                  valid = false;
                  break;
                }
                if (!class_day_feasible(classId, day)) {
                  valid = false;
                  break;
                }
                if (!subject_feasible(classId, cnd2.subj._id)) {
                  valid = false;
                  break;
                }
              }
            ...
            if (valid) {
              for (const p of placedData) {
                class_timetables[classId][day][hour] = p.combo_id;
                faculty_timetables[p.combo.faculty_id][day][hour] = p.combo_id;
                subject_hours_assigned_per_class[classId][p.subj._id]++;
              ...
              for (const p of placedData) {
                subject_hours_assigned_per_class[classId][p.subj._id]--;
                faculty_timetables[p.combo.faculty_id][day][hour] = EMPTY_SLOT;
